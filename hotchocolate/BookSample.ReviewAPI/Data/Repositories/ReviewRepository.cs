...
public class ReviewRepository
{
    ...
    private static readonly FrozenDictionary<long, IEnumerable<Review>> s_demoReviews = s_reviewFaker.GenerateForever()
        .Take(1000)
        .GroupBy(x => x.BookId)
        .ToFrozenDictionary(x => x.Key, x => x.AsEnumerable());

    private static readonly FrozenDictionary<long, Rating> s_demoRatings = s_demoReviews
